...
pub struct Response<D = ()> {
    ...
    on_send: Vec<Box<dyn FnMut(&mut Response<D>)>>
}
...
impl<D> Extensible for Response<D> {
...
impl<D> Pluggable for Response<D> {}
...
mod modifier_impls {
    ...
    impl<D> Modifier<Response<D>> for StatusCode {
        fn modify(self, res: &mut Response<D>) {
            *res.status_mut() = self
        }
    }

    impl<D> Modifier<Response<D>> for MediaType {
        fn modify(self, res: &mut Response<D>) {
